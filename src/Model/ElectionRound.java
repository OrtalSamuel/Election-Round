...
public class ElectionRound implements Serializable{
	...
	public ElectionRound(int round, LocalDate date) {
		//CONVERT TO SET -!-
		this.date = date;
		this.round = round;
	...
	public void setHardCode() {
		...
		updateHardCodeCandidateToParties();
		pollStationUnion();
	...
	public void sortNewCandidateByParty(Candidate c) {
		
		for (int i = 0; i < allParties.size(); i++) {
			if(c.belongToParty.equals(allParties.get(i).getPartyName()))
				allParties.get(i).setCandidateList(c);
		}
	}
	...
	public void updateHardCodeCandidateToParties() {
	...
	public Soldier createSoldier(String name, String id, int birthYear, boolean isVoting, String isWeapons) {
		...
		if(isWeapons.equals("y"))
			...
			allCitizen.add(s);
			sortNewVoterToPollStation(s);	
		return s;	
	}
	
//	
//	public void sortNewVoterToPollStation(Citizen c) {
//		
//		if(c.getSickDays() == 0 && c.getClass().getSimpleName().equals("Citizen"))
//			healthyStation.get(0).getAllowedCitizen().add(c);
//		
//		else if(c.getSickDays() != 0 && c.getClass().getSimpleName().equals("Citizen"))
//			coronaStation.get(0).getAllowedCitizen().add(c);
//		
//	
//		else if(c.getSickDays() == 0 && c.getClass().getSimpleName().equals("Soldier"))
//			soldierStation.get(0).getAllowedCitizen().add((Soldier) c);
//		
//		else if(c.getSickDays() != 0 && c.getClass().getSimpleName().equals("Soldier"))
//			sickSoldierStation.get(0).getAllowedCitizen().add((Soldier) c);
//
//	}
...
//		sortNewVoterToPollStation(cand);
//		sortNewCandidateByParty((Candidate) cand);
	}
	...
	public void updateSicknessForSoldierAndCitizen(int days, String isSuit) {
		...
		if(days != 0 && isSuit.matches("y") ) 
			allCitizen.get(allCitizen.size()-1).setSickDays(days);
		
		else if(isSuit.matches("y") && days != 0)
			allCitizen.remove(allCitizen.size());
	...
	public void divideCitizensToPollingStations() { 
		...
		for (int i = 0; i < allCitizen.size(); i++) {	
			...
			if(allCitizen.get(i).getSickDays() == 0 && allCitizen.get(i).getClass().getSimpleName().equals("Citizen"))
	...
	public void updateVoteToPollStation(String chooseParty, int days, String className) {
		...
		if(days != 0 && className.equals("Citizen"))
			coronaStation.get(0).votesStorage(chooseParty);
		
		else if(days == 0 && className.equals("Citizen"))
